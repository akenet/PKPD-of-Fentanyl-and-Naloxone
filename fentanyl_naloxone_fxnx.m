...
function [T, Y, Balance_f, Balance_n, AUC_f, AUC_n] = fentanyl_naloxone_fxnx(p, dose_f, time_f, num_f, delay, dose_n, time_n, num_n, time_start, y0)
% FENTANYL_NALOXONE_FXNX [T, Y, Balance_f, Balance_n, AUC_f, AUC_n] = fentanyl_naloxone_fxnx(dose_f, time_f, num_f, delay, dose_n, time_n, num_n, time_start, y0)
%   Function for adminstration of fentanyl and then naloxone (for repeated dosing scenarios).
%   Simulates repeated fentanyl administration followed by repeated naloxone administration.
%   "x" in "fxnx" represents number of doses for fentanyl (first "x") and naloxone (second "x").
...
if nargin == 8   % default case w/o time_start or y0
    time_start = 0;
...
tspan_f = [time_start, time_start + time_f]; % [min]  % time between fentanyl doses [mins]
...
if num_f == 1
    ...
    p.i_f = 1; % only one fentanyl administration
...
elseif num_f > 1
    ...
    % administer last dose of fentanyl
    % change timing
    if num_n == 0 
        tspan_btwn = tspan_f; % if naloxone not given, simulate for time until next fentanyl would have be given
    else
        tspan_btwn = [T_f(end), T_f(end) + delay]; % give naloxne "delay" minutes after last fentanyl administration
    end
    
    p.i_f = i+1; % keep track of fentanyl administrations
    
    % run simulation
    [Ti,Yi,Balance_f,Balance_n,AUC_f,AUC_n] = fentanyl_naloxone_main(tspan_btwn,y0,p);
...
tspan_n = [T_f(end), T_f(end) + time_n]; % [min]  % time between naloxone doses
...
for i = 1:num_n % number of doses of naloxone to administer
    ...
    % run simulation
    [Ti,Yi,Balance_f,Balance_n,AUC_f,AUC_n] = fentanyl_naloxone_main(tspan_n,y0,p);
    
    % reset initial conditions
    y0 = Yi(end,:);
    y0(4) = y0(4) + (p.dose_n_adj/p.Vd_n1);  % add the next naloxone dose as a concentration
    tspan_n = [Ti(end), Ti(end) + time_n];   % shift the time range for next administration
