...
# FIGURE 2 CODE -- Repeated Fentanyl
...
for(i in 1:4){
  ...
  setwd(paste0(root,'/Data/f25n0'))
  y_data <- readMat('Y_(f25n0).mat')
  t_data <- readMat('T_(f25n0).mat')
  ...
  if(i==1){
    plot1 <- ggplot(data = NULL) +
      ...
      scale_x_continuous(breaks = seq(0,1020,120)) +
      xlab('Time (min)') +
      ylab('[Drug] (nM)') +
...
ggsave('Fig2.png', plot = myPlot, width = 16, height = 12)
setwd(root)
